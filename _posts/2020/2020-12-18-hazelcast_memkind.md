...
easier for the community of software developers. One of our goals is to build convenient ways
of integrating Persistent Memory into software solutions as transparently as possible.
...
PMDK is a suite of software libraries designed to take advantage of Persistent Memory
exposed by the operating system, where PMem is presented as an independent and manageable memory resource.
Alternatively, you can use it transparently in conjunction with DRAM, which behaves like a cache.
This is called Memory Mode.
...
of the garbage collection algorithm used by the Java Virtual Machine.
...
which means the JVM suspends the application threads to ensure nothing mutates the heap
...
memory is done while the application threads are running - therefore,
the application threads allocate concurrently - it is possible that even these clever GC algorithms
are unable to keep up with the pace of the allocations and need to employ their so-called failure modes,
which is a well-controlled form of impacting the application to get the control back over
the allocation/reclamation dynamics. As the worst case option, these algorithms may need to suspend
application threads as well.
...
<figure class="image">
  <img src="/assets/hazelcast_1.png" alt="Segmentation of memory">
  <figcaption>Figure 1. Segmentation of the memory with HD.</figcaption>
</figure>

In 2019 Intel made its Optane Persistent Memory modules as the first publicly
available non-volatile memory product. While the biggest breakthrough
of these modules is their persistent capability, using them in a volatile App Direct
mode is attractive too. These modules are shipped with bigger capacity than that of the DRAM modules.
Therefore, the total achievable capacity is bigger than with the DRAM modules,
while the modules operate with near-DRAM performance, and for half of the price of the DRAM.
These advantages make the Optane PMM modules a good fit for Hazelcast and after some research,
Hazelcast 4.0 added support for the Optane modules - besides DRAM - as its HD backend.
The initial implementation in 4.0 used [libvmem][libvmem] for using Optane Persistent
...
with different performance characteristics. This solution enables the applications
to leverage Persistent Memory as the capacity tier of its memory resident data.
In the heterogeneous memory systems, where two or more memory types are present,
...
build a JavaNative Interface (JNI) bridge. Since Hazelcast’s HD memory manager
interacts with the memory allocators through a malloc-like internal abstraction,
it turned out that integrating Memkind was a very straightforward task,
since its API is very close to the standard memory allocation APIs. The notable difference
with Memkind is that the caller has to define a kind for an allocation, and optionally
needs to define the kind when freeing or reallocating a memory block. If the kind is not defined,
Memkind detects which kind should be used. This kind detection feature turned out to be a very
useful feature even for Hazelcast, which at the time of writing this post uses exclusively
either the `PMEM` or the `MEMKIND_DAX_KMEM_ALL` kind.

Looking forward, building a tiered HD storage, for example with DRAM and PMem tiers,
could be a feature for Hazelcast, to take advantage of the heterogeneous platform architectures.
...
Otherwise, various problems may occur potentially crashing the application. To deal with this problem,
one can track the allocator used for every memory block. The disadvantage of this approach
is a significant increase of the fragmentation. Since memkind internally tracks the dedicated memory
kind of a given object from the allocation metadata, it can detect the kind for every
object allocated with the library. This is a great help for the applications dealing
with heterogeneous memory resources as memkind eliminates the aforementioned problem entirely.
To learn more on kind detection, please see this [post][memkind_intro_post].

The partnership between the PMDK team and Hazelcast facilitates PMem integration into the
[High-Density Memory Store][high_density_memory_store]. The Hazelcast team leverages
the memkind library to explicitly allocate PMem space. With FS-DAX base memory kind,
the Hazelcast team analyzed memkind performance in the context of shutdown times and
found the bottleneck in the form of a punch hole operation when destroying file-based
memory kind. . Punch hole operation is used to deallocate blocks presented on filesystem
and is called during deallocating memory using memkind API so the underlying pages could
be returned to the OS. The PMDK team suggested using the mechanism `MEMKIND_HOG_MEMORY`
that controls the behavior of memkind in this regard, reducing the amount of memory
returned to the system, and instead allows the allocator to directly reuse the already
allocated memory pages. With this change the punch-hole operation was not called during
the free operation. After that, with the Hazelcast team, we discovered that the punch hole
operation when destroying a PMEM memory kind was made entirely redundant, which allowed us to
[entirely eliminate this operation][memkind_madvise_commit_link] for even greater performance gains,
resulting in a dramatic reduction of the shutdown times experienced by Hazelcast.


As another result of the collaboration between the teams, in the next release, libmemkind will
be improved by providing a method to verify if the path used to create a PMem memory kind point
on the DAX-aware filesystem, which would be previously possible only by using
additional dependency like [libpmem][libpmem]. With this change the memkind library will
become an even more standalone library.
...
On multi-socket systems there could be many persistent memory devices even if the DIMMs are interleaved,
one device for every socket. Figure 2 presents this filesystem-based solution which memkind
uses to support the PMem devices. Each PMem device is visible to the Operating System
as a block device based on the DAX-aware filesystem. The block device can be mounted
in various possible locations; therefore, the user must create a PMEM memory kind
for a defined path. In this mode, to utilize all PMem memory available in the system,
the application must make separated heaps for each socket (for each PMem device).
Besides managing the heaps separately, the application is also responsible for making
the allocations NUMA-aware. 
...
This realization made Hazelcast to implement its own heap unification with round-robin
...
## Thoughts on the performance of Optane

In the above sections we described the modes offered by memkind for allocating
volatile memory backed by PMem and how Hazelcast builds on these modes.
In the following we share some thoughts on the performance of PMem in general and
some of the key findings of the Hazelcast team from benchmarking Optane PMM in a
distributed environment.

If we go back to [one][pmem.io_perf_post] of the previous posts on the PMDK blog,
we see that PMem is a new tier between Memory and storage.
Apart from the data persistence and previously mentioned big capacity, the CPU can access
the data directly. Access to information in the physical medium is provided with cache-line granularity.
Besides memory capacity, PMem fulfills the gap between DRAM and storage in terms of latency.
It provides an asymmetric performance (see [section 12.2][arch_optimization]
and [Basic Performance Measurements][nvsl_analysis]) depending on the type of access operation
reading/write and characteristics of access sequentially/random.

Performance impact for remote NUMA access for Optane is much larger compared to DRAM
(see [section 5.4][guide_to_pmem]). To achieve the best performance, we need to limit
...
The performance of the Optane modules is in between the performance of DRAM modules
and storage devices. The bandwidth of Optane PMem is lower than the bandwidth of the DRAM modules,
while their access time is greater. This would make some think that Optane compared to DRAM
is a poor choice when it comes to performance. Based on Hazelcast’s tests,
this is not necessarily the case. Despite the performance handicap of the PMem devices,
in many use cases Hazelcast recorded performance results with Optane very similar to the results with DRAM.
There are multiple reasons for this. Since the persistent memory accesses are transparently cached
in the CPU cache layers just like the DRAM accesses are cached, the cache-locality techniques used
to speed up DRAM accesses work just as well for Optane. Hazelcast uses these techniques to exchange DRAM
and PMem accesses to cache hits where it is possible making it irrelevant if the used cache lines
are backed by DRAM or by Optane. Another reason lies in the fact that Hazelcast is a distributed system
working with many networking communications that involve serialization, in-memory copying, and internally,
inter-thread communication. There are also hiccups added by the JVM - just like GC pauses - and by the OS.
All of this incurs performance costs that are potentially in a higher range than the extra cost of accessing
Optane compared to accessing DRAM. In other words, operating in a distributed environment can hide the
performance disadvantage of Optane. Hazelcast’s main finding for achieving Optane’s performance parity
with DRAM was paying attention to the NUMA-locality. Figure 4 illustrates the potential gain that can be
achieved by NUMA-local Optane accesses in FS-DAX mode. The chart compares the throughput of a single Hazelcast
member with the same caching workload, but with different settings. The benchmark used for producing the chart
executed two operations: get and put operations in Hazelcast IMap (Hazelcast’s key-value data structure),
...
Optane accesses from 50% (blue line) to 100% (green line). It also shows that there
is no such big difference in the case of DRAM (pink and red lines). Besides the less
...
aspect of Optane PMem in an already existing software product from Hazelcast.
...
[hazelcast_blog]:https://THIS_LINK_SHOULD_BE_UPDATED.com
